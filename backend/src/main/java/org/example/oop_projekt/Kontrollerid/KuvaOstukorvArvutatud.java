...
import org.example.oop_projekt.DTO.OstukorvPoodDTO;
import org.example.oop_projekt.DTO.OstukorvTootedDTO;
...
public class KuvaOstukorvArvutatud {
    ...
    @PostMapping
    public ResponseEntity<Object> getToode(@PathVariable("id") Long id, @RequestBody TokenDTO token){
        Ostukorv ostukorv = ostukorvRepository.findOstukorvById(id);

        if (ostukorv != null) {
            return ResponseEntity.ok().body(
                    Map.of("sonum", "Ostukorv leitud", "tooteAndmed", ostukorvTeenus.looOstukorviDTO(ostukorv, token)));
        }
